...
- **Multi-cell tracking**: TRA 0.929 on CTC benchmark; biology-aware division annotator (`core/division_annotator.py`) detects pre-mitotic-swelling → balled → split → grown-daughter pattern, catching **1 / 2 GT divisions across 9 IC295 GT recordings** (Pos51_Y1). Pos39_OT regressed to 0/1 after the 2026-05-21 mirror-pad rollout — cpsam+padding now segments daughters into separate tracks before the parent's area halves, so the annotator's primary area-halving trigger doesn't fire. Needs a "backward-from-new-track" detection path (queued).
- **GT aggregate**: 9 recordings, mean per-cell IoU **0.858**, F1@.5 **0.80**, ID consistency **95.51%** (238 frames). IoU improved on all 9 after the 2026-05-21 mirror-pad rollout; F1 went up dramatically on the previously-worst recordings (Pos68_DMSO 0.50→0.56, Pos51_Y1 0.90→1.00) but dipped on small legacy ignasi where raw cpsam finds 2 extra real cells per frame that the single-cell GT doesn't annotate (scored as FPs by the IoU-based F1).

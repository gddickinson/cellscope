...
- **track_gap_fill.py** — Post-tracking gap fill cascade: cpsam(augment) → CP3 + MedSAM + DeepSea → SAM2 video propagation → translation-only fill. 100% fill rate on tested recordings (41/41 gaps). **Phase 1 cascades fast→slow**: cpsam on an adaptive crop, no-augment (`DEFAULTS.gap_fill_crop` / `gap_fill_augment`), with augment-on-miss then full-frame-on-miss fallbacks so recall ≥ the old always-full/always-augment path. GT-validated no quality loss (`scripts/bench_gap_fill.py`): crop 12.5× at full res; `augment=False` 2.1× at production 1024² (the lever that survives downsampling, since augment is call-count not pixels). `fill_track_gaps(..., use_crop, use_augment, stats_out)` logs per-phase timing + gaps-filled.

...
- **editor_help.py** — Mask-editor Help menu (`install_help_menu`), a grouped keyboard-shortcuts popup (`ShortcutsDialog`, source of truth `SHORTCUTS`), and an in-app guide dialog rendering `docs/mask_editor_guide.md` (`GuideDialog`, falls back to the GitHub link). `show_shortcuts` / `show_guide` / `open_online_docs` helpers.
